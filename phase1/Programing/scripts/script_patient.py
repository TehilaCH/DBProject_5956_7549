...
with open("patient_inserts.sql", "w", encoding='utf-8') as f:
    for i in range(1, 401):  # 400 פצועים בדיוק
        ...
        # יצירת hospital_id אקראי בין 1 ל-50 (ה-IDs אמורים להתאים לבתי חולים בטבלת Hospital)
        hospital_id = random.randint(1, 50)

        # יצירת הפקודה SQL
        sql = f"INSERT INTO Patient (patient_id, patient_name, birthday, patient_type, event_id, hospital_id) VALUES ({i}, '{patient_name}', '{birthday}', {patient_type}, {event_id}, {hospital_id});\n"
        f.write(sql)
