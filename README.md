...
![image](https://github.com/user-attachments/assets/579a64ff-471d-432c-9604-a45fa0f9f2d5)
 ...
 לאחר מכן מוספים טיפול שביצע אותו פרמדיק טיפול מסוג חירום ומשך זמן 25 דקות וזה אמור להפעיל את הטריגר אוטומטית שיעלה את הניסיון של הפרמדיק הזה 
...
ניתן ליראות שתצאת השאילתא שקולה להדפסה של הפונקציה.
...
בדיקה כאשר מכניסים קלטים שאין להם תצאה כמו בדוגמא הבאה שבה אנחנו רוצים להחזיר את החולים בבית חולים מספר 1 שעברו 1000 או יותר טיפולים ומודפס הודה שנדפס בחריגה:
"לא נמצאו פצועים בבית חולים 1 עם לפחות 1000 טיפולים"

![image](https://github.com/user-attachments/assets/e1c4910c-e93a-4787-8ba5-a193d340eb2b)

ההדפסה של מה שהפורצדורה מדפיסה את כל מי שעמד בתנאי ושינתה לו את הדרגה:
...
1.**מסך ראשי** – מאפשר גישה לכל מסכי המערכת- כפתורים לניהול טבלת מבצעים,טבלת ציוד וטבלת דרושים וכן כפתור למסך הפעלת פונקציה, פורצדורה ושאילתא על המסד נתונים.

2.**ניהול מבצעים** (Operation) – מאפשר הוספה, עדכון, מחיקה ושליפה של מבצעים.

3.**ניהול ציוד** (Equipment) – CRUD על טבלת הציוד.

4.**ניהול ציוד נדרש** (Requires) – טבלה מקשרת בין ציוד למבצעים.

5.**שאילתות ופרוצדורות** – הצגת פלטים מתוך פונקציות/פרוצדורות כולל:

*שאילתה על טיפולים לפי פרמדיק.

*שאילתא על כמות השימושים בציוד לפי חודש במהלך שנת 2023.

*הפעלת פונקציית refcursor למציאת פצועים בבית חולים.

*הפעלת פונקציה לעדכון הניסיון לפרמדיקים לפי כמות טיפולים.
